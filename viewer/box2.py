...
@timer_decorator
def train_shapecast_fast(points):
    col_first = [255, 230, 230]
    col_sec = [255, 180, 180]
    STEP = 5.0
    offset = np.array([0.0, 0.0, 0.0])
    ...
    with timer("Train Shapecast"):
        ...
        for i in range(30):
            origin = origin + np.array([0.0, -STEP, 0.0])
            # cast top
            hit_center, hit_dist, hit_points = perform_boxcast_fast(points, origin, direction, y_box_size, max_dist=5.0)
            if hit_center is None:
                return
            rr.log(f"world/start{r()}", rr.Boxes3D(centers=[hit_center], half_sizes=[y_box_size / 2], colors=[col_first]))
            rr.log(f"world/start{r()}", rr.Points3D(hit_points, radii=0.05, colors=col_first))
            ...
            # cast left
            direction = np.array([-1.0, 0.0, 0.0])
            hit_center, hit_dist, hit_points = perform_boxcast_fast(points, left_pos, direction, y_box_size, max_dist=15.0)
