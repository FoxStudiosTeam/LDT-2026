...
#ifndef OSRF_TESTING_TOOLS_CPP__DEMANGLE_HPP_
#define OSRF_TESTING_TOOLS_CPP__DEMANGLE_HPP_
...
std::string
demangle_str(const std::string & mangled_typeid_name)
{
#if !defined(_WIN32)
  int status = 0;
...
// Cannot do demangling if on Windows or if we want to avoid memory allocation.
  ...
  return demangle_str(mangled_typeid_name);
#else
...
#endif  // OSRF_TESTING_TOOLS_CPP__DEMANGLE_HPP_
