...
template<typename T, std::enable_if_t<std::is_same_v<std::decay_t<T>, test_msgs::msg::MultiNested>, int> = 0>
constexpr auto as_tuple_ref(T && msg)
{
  return std::forward_as_tuple(
    std::forward<T>(msg).array_of_arrays,
    std::forward<T>(msg).array_of_bounded_sequences,
    std::forward<T>(msg).array_of_unbounded_sequences,
    std::forward<T>(msg).bounded_sequence_of_arrays,
    std::forward<T>(msg).bounded_sequence_of_bounded_sequences,
    std::forward<T>(msg).bounded_sequence_of_unbounded_sequences,
    std::forward<T>(msg).unbounded_sequence_of_arrays,
    std::forward<T>(msg).unbounded_sequence_of_bounded_sequences,
    std::forward<T>(msg).unbounded_sequence_of_unbounded_sequences);
}
...
template<>
constexpr const char * data_type<test_msgs::msg::MultiNested>()
{
  return "test_msgs::msg::MultiNested";
}

template<>
constexpr const char * name<test_msgs::msg::MultiNested>()
{
  return "test_msgs/msg/MultiNested";
}
...
template<>
struct MessageTraits<test_msgs::msg::MultiNested>
{
