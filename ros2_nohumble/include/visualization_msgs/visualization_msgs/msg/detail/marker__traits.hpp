...
template<typename T, std::enable_if_t<std::is_same_v<std::decay_t<T>, visualization_msgs::msg::Marker>, int> = 0>
constexpr auto as_tuple_ref(T && msg)
{
  return std::forward_as_tuple(
    std::forward<T>(msg).header,
    std::forward<T>(msg).ns,
    ...
    std::forward<T>(msg).lifetime,
    std::forward<T>(msg).frame_locked,
    std::forward<T>(msg).points,
    std::forward<T>(msg).colors,
    std::forward<T>(msg).texture_resource,
    std::forward<T>(msg).texture,
    std::forward<T>(msg).uv_coordinates,
    std::forward<T>(msg).text,
    std::forward<T>(msg).mesh_resource,
    std::forward<T>(msg).mesh_file,
    std::forward<T>(msg).mesh_use_embedded_materials);
}
...
template<>
constexpr const char * data_type<visualization_msgs::msg::Marker>()
{
  return "visualization_msgs::msg::Marker";
}

template<>
constexpr const char * name<visualization_msgs::msg::Marker>()
{
  return "visualization_msgs/msg/Marker";
}
...
template<>
struct MessageTraits<visualization_msgs::msg::Marker>
{
