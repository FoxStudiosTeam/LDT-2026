use std::{sync::{Arc, RwLock, atomic::{AtomicUsize, Ordering}}, time::Duration};
...
pub async fn entry(mut point_cloud_stream : PointCloudStream, recording_stream : RecordingStream, point_cloud : Arc<RwLock<AppPointCloud>>) -> Result<(), Error> {
    ...
    while let Some(a) = point_cloud_stream.next().await? {
        frame_id = a;
        println!("message getted");

        // 2. Проверяем лимит без блокировок
        if active_tasks.load(Ordering::Relaxed) >= 4 {
            println!("Скипаем кадр {}, так как обработка перегружена", frame_id);
            continue; 
        }
        ...
        tokio::spawn(async move {
            ...
            let _guard = TaskGuard { counter: active_tasks_clone };
            let point_cloud = point_cloud.clone();
